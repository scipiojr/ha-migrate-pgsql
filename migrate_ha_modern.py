...
def action_convert_schema(config):
    ...
    targets = questionary.checkbox(
        "Wähle Tabellen zur Konvertierung:",
        choices=choices
    ).ask()
...
def run_migration_plan(config, plan):
    ...
    with PostgresManager(config, fast_mode=True) as p_conn:
        ...
        rich_progress = Progress(
            ...
            TextColumn("•"),
            TimeRemainingColumn(),
            TextColumn("• {task.completed}/{task.total} Zeilen"),
        )
    ...
    if choice.startswith("Alle"):
        ...
        for t in target_plan:
            if t in DEPENDENCIES:
                for parent in DEPENDENCIES[t]:
                    if parent not in selected_set: missing.add(parent)
        ...
        if missing:
            ...
            opt = questionary.select(
                "Wie verfahren?",
                choices=[
                    "Fehlende automatisch hinzufügen",
                    "Ignorieren (bereits migriert)",
                    "Abbruch"
                ]
            ).ask()
...
def action_maintenance(config):
    choice = questionary.select(
        "Wartungsoption wählen:",
        choices=[
            "Sequenzen Reset (Pre-Flight)",
            "DB Status Check (Vergleich)",
            "Postgres VACUUM",
            "Zurück"
        ]
    ).ask()
    ...
    if choice.startswith("Sequenzen"): action_sequence_reset(config)
        ...
        if questionary.confirm("Starten?").ask():
            try:
                ...
                cur = conn.cursor()
                with console.status("Führe VACUUM ANALYZE aus..."):
